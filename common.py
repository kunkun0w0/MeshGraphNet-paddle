...
def triangles_to_edges(faces):
    ...
    edges = paddle.concat([faces[:, :, 0:2], faces[:, :, 1:3],
                           paddle.stack([faces[:, :, 2], faces[:, :, 0]], axis=2)], axis=1)

    # those edges are sometimes duplicated (within the mesh) and sometimes
    # single (at the mesh boundary).
    # sort & pack edges as single tf.int64
    receivers = paddle.min(edges, axis=2)
    senders = paddle.max(edges, axis=2)
    packed_edges = paddle.cast(paddle.stack([senders, receivers], axis=2), paddle.int64)

    # remove duplicates and unpack
    unique_edges = paddle.cast(paddle.unique(packed_edges, axis=2), paddle.int32)
    senders, receivers = paddle.unstack(unique_edges, axis=2)
